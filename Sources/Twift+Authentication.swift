...
extension Twift {
  ...
  public class Authentication: NSObject, ASWebAuthenticationPresentationContextProviding {
    ...
    public func requestUserCredentials(
      ...
      var stepOneRequest = URLRequest(url: URL(string: "twitterauth://api.twitter.com/oauth/request_token")!)
      ...
      URLSession.shared.dataTask(with: stepOneRequest) { (requestTokenData, _, error) in
        ...
        let authURL = URL(string: "twitterauth://oauth/authorize?oauth_token=\(oauthToken)")!
        ...
        let authSession = ASWebAuthenticationSession(url: authURL, callbackURLScheme: callbackURLScheme) { (url, error) in
          ...
          if let url = url {
            ...
            var stepThreeRequest = URLRequest(url: URL(string: "twitterauth://oauth/access_token?oauth_verifier=\(oauthVerifier)")!)
